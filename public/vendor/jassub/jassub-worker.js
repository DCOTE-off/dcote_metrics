...
async function Module(moduleArg = {}) {
  ...
  var PThread = { unusedWorkers: [], runningWorkers: [], tlsInitFunctions: [], pthreads: {}, init() {
    ...
    var worker;
    worker = new Worker(new URL("jassub-worker.js", import.meta.url), { type: "module", name: "em-pthread" });
    PThread.unusedWorkers.push(worker);
  }, getNewWorker() {
  ...
  function loadModule() {
    ...
    WebAssembly.instantiateStreaming(fetch(new URL("jassub-worker.wasm", import.meta.url)), imports).then((output) => {
